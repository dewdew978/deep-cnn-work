...
* **ความเสถียรและความเบา (Efficiency):** ResNet-18 มีขนาดพารามิเตอร์เพียงประมาณ **11.2 ล้านตัว (ขนาดไฟล์ 44.9 MB)** จึงกินทรัพยากรน้อย ประมวลผลได้รวดเร็ว เหมาะสำหรับการใช้งานระดับ Real-time
...
train_transform = transforms.Compose([
    ...
    transforms.RandomAffine(degrees=0, translate=(0.06, 0.06), scale=(0.94, 1.06), fill=255),
...
2. **Random Affine (Translate +-6%, Scale 0.94-1.06x):** จำลองตัวอักษรที่อ้วน ผอม หรือเยื้องตำแหน่ง
...
#### จ) Universal Inference Pipeline & Image Support
* ระบบ Inference ได้รับการพัฒนาให้รองรับ:
  1. ชนิดไฟล์รูปภาพ: PNG, JPG, BMP, WEBP, และ GIF (จัดการ Animated GIF และพื้นหลังโปร่งใส Alpha Channel ให้อัตโนมัติ)
  2. ชนิดไฟล์ตาราง: CSV, XLSX, XLS
  3. รูปแบบตารางเฉพาะ: มีสมุดโค้ด `infer_teacher.ipynb` รองรับโครงสร้าง `Path | Image Name | Extension | Label | Result` ประกอบพาธรูปภาพและเติมคำตอบลงในตารางของอาจารย์ได้ทันที
...
### 11. สรุปผลและข้อเสนอแนะในการนำไปใช้งานจริง (Conclusion & Future Work)

#### ก) สรุปผลสัมฤทธิ์ของโครงงาน
...
#### ข) ข้อเสนอแนะในการพัฒนาต่อยอด (Future Work)
1. **การบูรณาการเข้ากับระบบ Full-Page OCR:**
   - นำแบบจำลองนี้ไปเชื่อมต่อกับ Text Detection Module (เช่น CRAFT, DBNet หรือ YOLOv8) เพื่อตัดคำและอักขระจากเอกสารทั้งหน้าก่อนส่งเข้าจำแนก
2. **การรู้จำข้อความลายมือเขียน (Handwritten Character Recognition):**
   - เพิ่มชุดข้อมูลลายมือเขียนภาษาไทยจริงเพื่อขยายขอบเขตการใช้งานไปยังเอกสารกรอกมือหรือแบบฟอร์มราชการ
3. **การแปลงแบบจำลองเพื่อการใช้งานบน Edge Devices:**
   - ทำ Quantization (INT8) และแปลงเป็น ONNX หรือ TensorRT เพื่อติดตั้งบนอุปกรณ์พกพา, กล้องวงจรปิด, หรือโมบายล์แอปพลิเคชันที่มีข้อจำกัดด้านพลังงาน
